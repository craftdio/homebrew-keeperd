...
After publishing a KeepERD GitHub Release, run **Actions → Update KeepERD Formula → Run workflow** with `main` selected. Other branches are rejected. Enter the version without `v` (for example, `0.1.3`). The workflow checks the maintainer's GitHub identity, verifies the release archive against its published checksum, then installs and tests the updated Formula on macOS. Validation and branch creation use the same `main` commit from the start of the run. Only after those checks pass does it commit the Formula to a `formula/keeperd-vX.Y.Z` branch. Review the branch and open a pull request manually before merging it into `main`.
